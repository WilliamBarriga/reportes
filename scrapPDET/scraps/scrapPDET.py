...
async def scrapPDET(region: int = 0):
    ...
    try:
        ...
        subregion= driver.find_element(By.ID, 'SubregionesPDET')
        subregion.click()
        logging.info('click PDET')
        driver.implicitly_wait(5)
        subregionselect = driver.find_element(By.CSS_SELECTOR, '#contentSubRegiones #DirTerritorial')
        drop_options = Select(subregionselect)
        logging.info('select')
        drop_options.select_by_index(region)
        driver.implicitly_wait(5)
